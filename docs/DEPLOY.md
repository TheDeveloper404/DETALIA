...
### Backup automat zilnic (GitHub Actions, independent de Neon)

`.github/workflows/db-backup.yml` — rulează zilnic la 03:00 UTC (`cron: "0 3 * * *"`, redus de la orar
2026-07-14 — orar dădea ~720 backup-uri simultane păstrate, zgomot inutil pentru volumul actual), `pg_dump`
pe `production` (`PROD_DATABASE_URL_BACKUP`, binarul v18 explicit — Ubuntu runner are implicit v16 în PATH,
mismatch cu serverul), format custom, urcat ca artifact GitHub Actions (retenție 30 zile → ~30 backup-uri
simultane). Complementar ferestrei native Neon (§ de mai sus, 6h) — acoperă orizontul de o lună fără să
depindă de planul Neon. Repo public → Actions minute/storage gratuite, nelimitate (nu e cost, doar igienă).
