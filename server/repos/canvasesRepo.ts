...
// Ownership-ul (owner_id === user din sesiune) îl aplică mereu canvasService ÎNAINTE / prin guard în `where`.
...
// Salvează snapshot-ul (autosave), condiționat pe owner. Întoarce thumbnailUrl-ul curent = irelevant aici;
// întoarce doar dacă a modificat un rând.
export async function updateStateOwned(
  id: string,
  ownerId: string,
...
// Id-urile detaliilor din index (pentru reconcilierea placeholder-elor la load: care mai sunt accesibile).
